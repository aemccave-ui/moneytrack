...
[[ "$APPLY" -eq 1 ]] || { echo 'SPC001_F4_ACCEPTANCE_POLISH_PREVIEW=REFUSED explicit_--apply_required' >&2; exit 2; }
...
for path in "${CHANGED[@]}"; do
  case "$path" in
    .github/workflows/spc001-source-contract.yml|miniapp/src/main.jsx|miniapp/src/spc001-f4-acceptance-polish.css|scripts/spc001-f4-acceptance-polish-source-gate.py|scripts/spc001-f4-acceptance-polish-preview-apply.sh|scripts/spc001-f4-financial-transport-apply.sh|scripts/spc001-f4-financial-transport-source-gate.py|scripts/spc001-f4-financial-transport-verify.py|scripts/spc001-generate-financial-api.py) ;;
    *) echo "ERROR: non-F4.2 source changed after accepted F4 preview: $path" >&2; exit 1 ;;
  esac
done
[[ " ${CHANGED[*]} " == *" miniapp/src/main.jsx "* ]]
[[ " ${CHANGED[*]} " == *" miniapp/src/spc001-f4-acceptance-polish.css "* ]]
echo "F4_TO_F4R2_SAFE_DELTA=PASS files=${#CHANGED[@]}"
...
rollback() {
  ...
  echo "SPC001_F4_ACCEPTANCE_POLISH_PREVIEW=FAIL rc=$rc" >&2
  exit "$rc"
}
trap rollback ERR
...
curl -fsS -H 'Cache-Control: no-cache' "$PREVIEW_URL/?spc001f4r2=$STAMP" -o "$REMOTE_INDEX"
[[ -s "$REMOTE_INDEX" ]]
...
while IFS= read -r asset; do
  [[ -n "$asset" ]] || continue
  tmp="$(mktemp)"
  curl -fsS -H 'Cache-Control: no-cache' "$PREVIEW_URL$asset?spc001f4r2=$STAMP" -o "$tmp"
...

{
  ...
  echo 'ACCEPTANCE_DELTA=RECOVERY_NOTICE_TRANSIENT,HOME_FAB_NON_OVERLAY'
  echo 'DB_MUTATION=NONE'
  echo 'N8N_MUTATION=NONE'
  echo 'PRODUCTION_FRONTEND_MUTATION=NONE'
  echo 'PREVIEW_MUTATION=APPLIED'
  echo 'SPC001_F4_ACCEPTANCE_POLISH_PREVIEW=PASS'
...
echo "SPC001_F4_ACCEPTANCE_POLISH_EVIDENCE_DIR=$OUTPUT_DIR"
echo 'DB_MUTATION=NONE'
echo 'N8N_MUTATION=NONE'
echo 'PRODUCTION_FRONTEND_MUTATION=NONE'
echo 'PREVIEW_MUTATION=APPLIED'
