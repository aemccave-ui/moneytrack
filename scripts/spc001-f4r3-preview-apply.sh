...
[[ "$APPLY" -eq 1 ]] || { echo 'SPC001_F4R3_PREVIEW=REFUSED explicit_--apply_required' >&2; exit 2; }
...
for path in "${CHANGED[@]}"; do
  case "$path" in
    ...
    *) echo "ERROR: non-F4 corrective source changed after accepted F4 preview: $path" >&2; exit 1 ;;
  esac
done
[[ " ${CHANGED[*]} " == *" miniapp/src/quick-actions-runtime.js "* ]]
[[ " ${CHANGED[*]} " == *" miniapp/src/spc001-f4-acceptance-polish.css "* ]]
echo "F4_TO_F4R3_SAFE_DELTA=PASS files=${#CHANGED[@]}"
...
rollback() {
  ...
  echo "SPC001_F4R3_PREVIEW=FAIL rc=$rc" >&2
  exit "$rc"
}
trap rollback ERR
...
curl -fsS -H 'Cache-Control: no-cache' "$PREVIEW_URL/?spc001f4r3=$STAMP" -o "$REMOTE_INDEX"
[[ -s "$REMOTE_INDEX" ]]
...
while IFS= read -r asset; do
  [[ -n "$asset" ]] || continue
  tmp="$(mktemp)"
  curl -fsS -H 'Cache-Control: no-cache' "$PREVIEW_URL$asset?spc001f4r3=$STAMP" -o "$tmp"
...

{
  ...
  echo 'ACCEPTANCE_DELTA=RECOVERY_NOTICE_TRANSIENT,HOME_ADD_DOCKED_TO_RECENT_OPERATIONS'
  echo 'DB_MUTATION=NONE'
  echo 'N8N_MUTATION=NONE'
  echo 'PRODUCTION_FRONTEND_MUTATION=NONE'
  echo 'PREVIEW_MUTATION=APPLIED'
  echo 'SPC001_F4R3_PREVIEW=PASS'
...
echo "SPC001_F4R3_PREVIEW_EVIDENCE_DIR=$OUTPUT_DIR"
echo 'DB_MUTATION=NONE'
echo 'N8N_MUTATION=NONE'
echo 'PRODUCTION_FRONTEND_MUTATION=NONE'
echo 'PREVIEW_MUTATION=APPLIED'
