...
BACKUP_DIR="${MONEYTRACK_UX023_BACKUP_DIR:-/var/backups/moneytrack/ux023-receipt-accounting/$STAMP}"
WORK="$(mktemp -d /tmp/ux023-receipt-accounting.XXXXXX)"
...
echo '# Phase'
echo 'UX-023 receipt accounting controlled backend + preview apply'
echo '# Gate'
...
import_publish() {
  local file="$1"
  local id="$2"
  local remote="/tmp/ux023-accounting-$(basename "$file")"
...
export_workflow() {
  local target="$1"
  local remote="/tmp/ux023-accounting-export-$$.json"
  docker exec -u 0 "$N8N_CONTAINER" rm -f "$remote" >/dev/null 2>&1 || true
...
verify_old_currency_route_absent() {
  local code
  code="$(curl -sS -X PATCH -H 'Content-Type: application/json' -d '{}' -o "$WORK/old-route-check.json" -w '%{http_code}' "$API_BASE/api/v1/receipt/currency" || true)"
  [[ "$code" == '404' ]] || { echo "old_currency_route_absent=FAIL http=$code" >&2; return 1; }
  echo 'old_currency_route_absent=PASS http=404'
}

rollback_runtime() {
  echo 'UX023_ACCOUNTING_ROLLBACK=START' >&2
...
drop function if exists moneytrack.receipt_update_accounting_v1(bigint,bigint,bigint,text);
...
on_exit() {
  ...
  if (( ! SUCCESS )) && (( DB_MUTATED || N8N_MUTATED || PREVIEW_MUTATED )); then
    echo "UX023_RECEIPT_ACCOUNTING_APPLY=FAIL status=$status" >&2
    rollback_runtime
  fi
  rm -rf "$WORK"
  exit "$status"
...
python3 "$ROOT/scripts/ux023-verify-receipt-editor.py"
...
where n.nspname = 'moneytrack'
  and p.oid in (
    ...
    to_regprocedure('moneytrack.receipt_update_accounting_v1(bigint,bigint,bigint,text)'),
...
python3 "$ROOT/scripts/ux023-generate-receipt-accounting-workflow.py" --output "$WORK/workflow.candidate.json"
...
ux022_db_psql_file "$ROOT/db/domain/UX-023/020_receipt_accounting_hardening.sql"
...
select 'receipt_accounting_write=' || case when to_regprocedure('moneytrack.receipt_update_accounting_v1(bigint,bigint,bigint,text)') is not null then 'PRESENT' else 'ABSENT' end;
select 'receipt_currency_only_write=' || case when to_regprocedure('moneytrack.receipt_set_currency_v1(bigint,bigint,text)') is null then 'ABSENT' else 'PRESENT' end;
...
grep -qx 'receipt_accounting_write=PRESENT' <<<"$DB_VERIFY"
grep -qx 'receipt_currency_only_write=ABSENT' <<<"$DB_VERIFY"
...
remote_html="$(curl -fsS -H 'Cache-Control: no-cache' "$PREVIEW_URL/?ux023accounting=$STAMP")"
...
remote_tmp="$(mktemp)"
curl -fsS -H 'Cache-Control: no-cache' "$PREVIEW_URL$remote_asset?ux023accounting=$STAMP" -o "$remote_tmp"
...
echo 'DB_MUTATION=UX023_RECEIPT_ACCOUNTING_BOUNDARIES_APPLIED'
echo 'N8N_MUTATION=UX023_RECEIPT_ACCOUNTING_API_PUBLISHED'
echo 'PREVIEW_FRONTEND_MUTATION=APPLIED'
echo 'PRODUCTION_FRONTEND_MUTATION=NONE'
echo 'UX023_RECEIPT_ACCOUNTING_APPLY=PASS'
