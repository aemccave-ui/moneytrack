...
require('currency_chip_visual_contract', '.currencyGroupHeader' in text('miniapp/src/accounts-explorer.css') and '.accountTreeRow' in text('miniapp/src/accounts-explorer.css'), 'currency/account visual language shares shell semantics')
require('currency_count_badge', 'homeCountBadge' in text('miniapp/src/App.jsx') and 'group.accounts.length' in text('miniapp/src/App.jsx'), 'currency badge uses account count')
require('account_count_badge', 'leafCount' in text('miniapp/src/App.jsx') and 'homeCountBadge' in text('miniapp/src/App.jsx'), 'account badge uses descendant leaf count')
...
require('year_period_present', "period === 'year'" in explorer and '>Год</button>' in explorer and "dateFrom: `${year}-01-01`" in periods, 'year is a canonical period')
require('previous_next_week', 'shiftPeriod(period, current, direction)' in explorer and "periodType === 'week'" in periods, 'week shifts anchor by calendar week')
require('previous_next_month', 'shiftPeriod(period, current, direction)' in explorer and "periodType === 'month'" in periods, 'month shifts anchor by calendar month')
require('previous_next_year', 'shiftPeriod(period, current, direction)' in explorer and "periodType === 'year'" in periods, 'year shifts anchor by calendar year')
require('single_period_model', 'resolvePeriod(period, anchorDate' in explorer and 'export function resolvePeriod' in periods and 'export function shiftPeriod' in periods, 'one shared period abstraction')
...
require('production_frontend_not_targeted', 'production' not in recent.lower() and 'production' not in explorer.lower(), 'UX-024 frontend source contains no deployment target mutation')
