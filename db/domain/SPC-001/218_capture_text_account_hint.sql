...
begin
    ...
    if nullif(btrim(p_existing_account_hint),'') is not null then
        return p_existing_account_hint;
    end if;
...
is 'SPC-001 Text Processor account-hint inference. Active membership is asserted and account-name matching is restricted to the requested Space while preserving the accepted raw-text matching semantics.';
