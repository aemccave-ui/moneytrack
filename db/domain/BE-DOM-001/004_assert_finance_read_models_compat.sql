-- MoneyTrack — BE-DOM-001 — hard parity assertions (portable psql variant)
--
-- This script avoids the psql \error meta-command, which is unavailable on
-- some deployed psql client versions. Failure is raised by PostgreSQL itself
-- through DO/RAISE EXCEPTION inside psql \if branches.
--
...
-- Ensure the selected verification user exists.
select exists (
    select 1
    from moneytrack.app_users
    where id = :user_id::bigint
) as verification_user_exists
\gset

\if :verification_user_exists
\else
  do $$ begin raise exception 'BE-DOM-001 assertion failed: verification user does not exist'; end $$;
\endif
...
\if :fx_parity
\else
  do $$ begin raise exception 'BE-DOM-001 assertion failed: FX parity mismatch'; end $$;
\endif
...
\if :accounts_parity
\else
  do $$ begin raise exception 'BE-DOM-001 assertion failed: accounts read-model parity mismatch'; end $$;
\endif
...
\if :dashboard_parity
\else
  do $$ begin raise exception 'BE-DOM-001 assertion failed: dashboard read-model parity mismatch'; end $$;
\endif
