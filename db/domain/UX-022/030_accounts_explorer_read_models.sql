...
begin
    ...
    external_period_transfers as (
        select tr.id
        from moneytrack.transfers tr
        left join selected_accounts sf on sf.id = tr.from_account_id
        left join selected_accounts st on st.id = tr.to_account_id
        where tr.user_id = v_user_id
          and tr.transfer_date >= p_date_from
          and tr.transfer_date < (p_date_to + 1)::date
          and ((sf.id is not null) <> (st.id is not null))
    ),
    period_summary as (
        select
    ...
    select
        ...
        (select count(*) from converted_balances cb where cb.balance_base is null)::bigint as snapshot_missing_rate_count,
        ps.income,
        ps.expense,
        (ps.income - ps.expense)::numeric,
        (ps.tx_count + (select count(*) from external_period_transfers))::bigint,
        p_date_from,
        p_date_to
    from user_ctx uc
