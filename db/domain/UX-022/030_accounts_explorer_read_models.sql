-- MoneyTrack — UX-022 — Accounts Explorer read models v2
-- Snapshot balances are valued at date_to. Period/category filters affect turnovers only.
...
begin
    ...
    select
        v_user_id,
        uc.base_currency,
        coalesce(sum(cb.balance_base) filter (where sa.id is not null),0)::numeric as total_base,
        coalesce(jsonb_agg(
            jsonb_build_object(
        ...
        count(*) filter (where cb.balance_base is null)::bigint as snapshot_missing_rate_count,
        ps.income,
        ps.expense,
        (ps.income - ps.expense)::numeric,
        ps.tx_count,
        p_date_from,
        p_date_to
    from user_ctx uc
    cross join period_summary ps
    cross join converted_balances cb
    left join selected_accounts sa on sa.id = cb.account_id
    group by uc.base_currency, ps.income, ps.expense, ps.tx_count;
end;
$function$;
...
begin
    ...
    raw_scope as (
        ...
        where child.user_id=v_user_id and coalesce(child.is_active,true)=true and p_include_descendants
    ),
    scope_accounts as (
    ...
    tx_converted as (
        select s.*,
            case when upper(s.source_currency)=upper(s.base_currency) then s.amount_original
                 else moneytrack.finance_fx_convert_usd_bridge_v1(s.amount_original,upper(s.source_currency),upper(s.base_currency),s.transaction_date::date)
            end as amount_base
        from tx_source s
    ),
    transfer_source as (
        select
            tr.id,
            case when sa_from.id is not null then tr.from_account_id else tr.to_account_id end as account_id,
    ...
    transfer_converted as (
        select s.*,
            case when upper(s.source_currency)=upper(s.base_currency) then s.amount_original
                 else moneytrack.finance_fx_convert_usd_bridge_v1(s.amount_original,upper(s.source_currency),upper(s.base_currency),s.transfer_date::date)
            end as amount_base
    ...
    transfer_summary as (
        select coalesce(sum(amount_base),0)::numeric as transfer_base,
    ...
    all_items as (
        select * from tx_items union all select * from transfer_items
    ),
    items as (
    ...
    select
        ...
        sm.scope_count,
        case when sm.scope_count=1 then s.income_original else s.income_base end,
        case when sm.scope_count=1 then s.expense_original else s.expense_base end,
        case when sm.scope_count=1 then s.income_original-s.expense_original else s.income_base-s.expense_base end,
        ts.transfer_base,
        ...
        i.transactions
    from user_ctx uc cross join summary s cross join transfer_summary ts cross join items i cross join scope_meta sm;
end;
$function$;

comment on function moneytrack.api_accounts_explorer_summary_read_model_v2(bigint,bigint[],bigint[],bigint[],date,date,date)
is 'UX-022 dated own-account snapshots plus period/category turnovers. date_from never affects snapshot; adjustment is expense turnover.';
comment on function moneytrack.api_transactions_read_model_v2(bigint,bigint,date,date,boolean,bigint[],bigint[],bigint[])
is 'UX-022 account/subtree operations read model. Includes parent own+descendant postings and suppresses transfers whose both endpoints are inside the selected scope.';
