-- UX-022 Accounts Explorer read-model hardening.
-- Keep full account_balances for the tree, but selected totals must not be blocked
-- by an FX gap that belongs only to an excluded account.
...
is 'UX-022 Accounts Explorer summary v2. Full account snapshot is returned for the tree; total_base and snapshot_missing_rate_count are scoped only to selected accounts.';
