...
export default function AccountsFilters({
  ...
  const categorySummary = allCategoryIds.length === 0 || (
    sameIds(effectiveIncomeIds, allCategoryIds) && sameIds(effectiveExpenseIds, allCategoryIds)
  ...
  const savePreset = async () => {
    const name = String(window.prompt('Название нового пресета') || '').trim()
    if (!name) return
    try {
  ...
  const renamePreset = async (preset) => {
    const name = String(window.prompt('Новое название', preset.name || '') || '').trim()
    ...
    <>
      <div className="accountsFilterActions">
        ...
        <button type="button" className="accountsFilterAction" onClick={() => setSheet('presets')} disabled={loading}>
          <span className="accountsFilterIcon bookmarkIcon" aria-hidden="true">♧</span>
            ...
            {categoryRows.map((category) => {
              ...
              return (
                <div className="categoryMatrixRow" key={id} style={{ '--category-depth': category.depth || 0 }}>
                  ...
                  <button type="button" className={draftIncome.has(id) ? 'isOn' : ''} onClick={() => toggleDraft(setDraftIncome, id)} aria-label={`${category.name || category.code}: доход`}>{draftIncome.has(id) ? '✓' : ''}</button>
                  <button type="button" className={draftExpense.has(id) ? 'isOn' : ''} onClick={() => toggleDraft(setDraftExpense, id)} aria-label={`${category.name || category.code}: расход`}>{draftExpense.has(id) ? '✓' : ''}</button>
                </div>
              )
            })}
          </div>
