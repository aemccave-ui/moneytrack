...
## Off-host and operator alerting

PROD-H4 resolves these controls separately. Until that gate is complete, local backup is proven but remains on the same host failure domain, and no external operator notification channel should be assumed.
